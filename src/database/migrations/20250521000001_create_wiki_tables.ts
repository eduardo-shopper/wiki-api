...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('articles', (table: TB) => {
    ...
    // (synonyms, business aliases). Included in the FTS index.
    ...
    // Populated on create/update when OPENAI_API_KEY is set.
    table.specificType('embedding', 'vector(1536)').nullable()
  })

  // GIN index on tsvector for fast full-text search in Portuguese.
  // Spans title + summary + keywords + content so keyword and semantic-ish
  // text queries all hit a single index.
  await knex.raw(`
  ...
  // HNSW index for cosine-distance vector similarity search.
  // Works on empty tables; no need to rebuild after bulk inserts (unlike IVFFlat).
  await knex.raw(`
  ...
  await knex.schema.createTable('article_tags', (table: TB) => {
    ...
    table.integer('id_tag').notNullable()
